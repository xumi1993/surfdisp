...
!c       02 Jul 2024 -- Modified by Shijie Hao, output hti kernel, output kernel to dimensionless parameters
!c- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
! model param
integer                                 :: mmax
real(kind=dp),dimension(:),ALLOCATABLE :: zd,zrho,za,zb,xmu,xlam
real(kind=dp)                          :: uu0(4)

! eigen function
real(kind=dp),dimension(:),ALLOCATABLE ::ur,uz,tz,tr, &
                                           dcda,dcdb,dcdr,dcdh,dcdgc,dcdgs
integer,DIMENSION(:),ALLOCATABLE :: iwat

! sumi
real(kind=dp)                          ::sumi0,sumi1,sumi2,sumi3,&
                                          flagr,are,ugr
! spherical earth
real(kind=dp),ALLOCATABLE              :: vtp(:),dtp(:),rtp(:)

! haskel matrix
real(kind=dp),ALLOCATABLE              :: exe(:),exa(:),cd(:,:),vv(:,:)

! allfluid
logical                         :: allfluid

! hwat
real(kind=dp),ALLOCATABLE              :: wh(:,:),hex(:)

! save
real(kind=dp)                          :: dd(5,5),aa(4,4),ex1,ex2
real(kind=dp),ALLOCATABLE              :: ww(:),xx(:),yy(:),zz(:),&
                                           cospp(:),cosqq(:)

! engerw wra,wd,wba
real(kind=dp)                          :: wra,wd,wba

!common/emat/e,einv,ra,rb
complex(kind=dp)               :: ra,rb
complex(kind=dp)               :: e(4,4), einv(4,4)

! openmp wrapper
!$omp threadprivate(mmax,zd,zrho,za,zb,xmu,xlam,uu0)
!$omp threadprivate(ur,uz,tz,tr,dcda,dcdb,dcdr,dcdh,dcdgc,dcdgs)
...
subroutine Alloc_Model(nlayer)
  ...
  ALLOCATE(dcda(mmax),dcdb(mmax),dcdh(mmax),dcdr(mmax),dcdgs(mmax),dcdgc(mmax))
  ALLOCATE(iwat(mmax))
...
subroutine Dealloc_Model()
  ...
  DEALLOCATE(ur,uz,tz,tr,dcda,dcdb,dcdr,dcdh,dcdgs,dcdgc)
  DEALLOCATE(iwat)
...
subroutine bldsph()
  ...
  implicit none 
  real(kind=dp)        ::  ar, dr, r0, r1, z0, z1
  real(kind=dp)        :: tmp
  integer        :: i
  ar=6371.0d0
  dr=0.0d0
...
subroutine svfunc(omega,wvno)
  ...
  use RayleighWaveModel, only :  ur,uz,tz,tr,uu0,mmax,allfluid,&
                                 cd,vv,exe,exa,iwat
  implicit none
  real(kind=dp),INTENT(IN)   :: omega, wvno
  ! internal variables

  real(kind=dp)              :: fr,uu1, uu2, uu3, uu4, ext, f1213, fact
  real(kind=dp)              :: cd1, cd2, cd3, cd4, cd5, cd6
  real(kind=dp)              :: tz1, tz2, tz3, tz4
  integer              :: i, jwat
...
subroutine up(omega,wvno,fr)
  ...
  ! local variables
  real(kind=dp)                    :: wvno2, om2,cr, ee(5), exn,&
                                      ca(5,5),xka, xkb, pex, svex
  real(kind=dp)                    :: cosp,rsinp,sinpr,cossv,& 
                                        rsinsv,sinsvr,exsum
  complex(kind=dp)         ::rp, rsv, p, q,gbr(2,5)
...
subroutine dnka(CA,cosp,rsinp,sinpr,cossv,rsinsv,sinsvr,&
  ...
  implicit none
  real(kind=dp)        ::  ca(5,5), wvno, wvno2, om2
  real(kind=cr)         :: rho,b
  real(kind=dp)        :: cosp,rsinp,sinpr,cossv,rsinsv,sinsvr
  integer        :: iwat

  ! local
  real(kind=cr)         :: rho2
  real(kind=dp)        :: gam,ex, exa,a0,cpcq,cpy,&
                            cpz,cqw,cqx,xy,xz,wy,wz
  real(kind=dp)        :: gam2,gamm1,gamm2,a0c,xz2,wy2,temp,&
...
subroutine evalg(jbdry,m,m1,gbr,inp,&
  ...
  complex(kind=dp) :: gbr(2,5) 
  real(kind=dp)            :: wvno, wvno2, om, om2

  !internal parameters
  real(kind=dp)            :: xka,xkb,gam,gamm1
...
subroutine varsv(p,q, rp, rsv, & 
  ...
  implicit none
  complex(kind=dp)   :: p,q,rp,rsv
  real(kind=dp)              :: cosp, cosq, rsinp, rsinq, &
                                 sinpr,sinqr,pex,svex,zd
  integer              :: iwat
  real(kind=dp)              :: pr, pi, qr, qi,PFAC, SVFAC
  complex(kind=dp)   :: epp, epm, eqp, eqm,sinp, sinq
...
subroutine hska(AA,cosp,rsinp,sinpr,tcossv,trsinsv,tsinsvr, &
  ...
  real(kind=dp)        :: AA(4,4),cosp , tcossv, rsinp, trsinsv,&
                            sinpr, tsinsvr
  real(kind=cr)         :: rho,b
  integer        :: iwat
  real(kind=dp)        :: pex, svex,om2, wvno, wvno2

  !local variables
  real(kind=dp)        :: gam, gamm1,cossv,rsinsv,sinsvr,dfac
...
subroutine down(omega,wvno)
  ...
  use RayleighWaveModel, only   : mmax,zd,za,zb,zrho,iwat,vv,&
                                  exa
  implicit none
  real(kind=dp)                :: omega,wvno

  !internal variables
  real(kind=dp)                :: wvno2, om2,aa(4,4),xka,xkb,pex,svex,&
                                    ex2,cosp,rsinp,sinpr,cossv,rsinsv,sinsvr,&
                                    cc,aa0(4),exsum
  complex(kind=dp)     :: rp,rsv,p,q
  integer                :: m,i,j
  om2 = omega*omega
  wvno2 = wvno*wvno
...
subroutine energy(om,wvno,mmax)
  ...
  use RayleighWaveModel,only : sumi0,sumi1,sumi2,sumi3,iwat,&
                               ...
                               ugr,dcdgc,dcdgs
  implicit none
  real(kind=dp)            :: om,wvno
  integer            :: mmax

  !internal variables
  real(kind=dp)            :: om2, wvno2,TA, TC, TF, TL, TN
  real(kind=dp)            :: INT11, INT13, INT22, INT44, INT33, INT24
  real(kind=dp)            :: URUR, UZUZ, DURDUR, DUZDUZ, URDUZ, UZDUR
  real(kind=dp)            :: c,fac,facah, facav, facbh, facbv,facr
  
  integer            :: m
  !coefficients of the ODE
  real(kind=dp)            :: a12, a14, a21, a23
  real(kind=dp)            :: ah, av, bh, bv, eta, rho
  ...
  do m=1,mmax
    ...
    if(iwat(m).eq.1)then
      ...
      facah = rho*ah*ah*(URUR -2.*eta*URDUZ/wvno)
      facav = rho*av*av*DUZDUZ / wvno2
      dcda(m) = facah + facav
      facr = -0.5*c*c*(URUR + UZUZ)
      dcdr(m) = 0.5*(av*facav + ah*facah ) + facr*rho
      dcdgc(m) = 0.0  ! no shear in fluid
      dcdgs(m) = 0.0  ! no shear in fluid
      ...
      facav = rho*av*av*DUZDUZ / wvno2
      facbh = 0.0
      facbv = rho*bv*bv*(UZUZ + 2.*UZDUR/wvno + DURDUR/wvno2 + &
                 4.*eta*URDUZ/wvno )
      dcda(m) = facah + facav
      dcdb(m) = facbv + facbh
      ...
      dcdr(m) = 0.5*(av*facav + ah*facah + bv*facbv) + facr*rho
      dcdgc(m) = rho*bv*bv*(UZUZ + 2.*UZDUR/wvno + DURDUR/wvno2) + &
                rho*bv*bv*(URUR)
      dcdgs(m) = dcdgc(m)
  ...
  do m=1,mmax
    ...
    dcdgs(m) = dcdgs(m) /(ugr*sumi0)
    dcdgc(m) = dcdgc(m) /(ugr*sumi0)
  enddo
...
function intijr(i,j,m,typelyr,om,om2,wvno,wvno2)
  ...
  implicit none
  real(kind=dp)            :: intijr
  integer            :: i,j,m,TYPELYR
  real(kind=dp)            :: om, om2, wvno,wvno2
  !TYPELYR = -1  layer represents upper halfspace
  ...
  !c-----
  !internal variables
  complex(kind=dp)       :: cintijr, FA, GA, FB, GB, H1, H2
  complex(kind=dp)       :: gbr(2,5),kmpu, kmsu, km1pd, km1sd
...
function ffunc(nub,dm)
  implicit none
  complex(kind=dp)       :: ffunc,nub
  real(kind=dp)                  :: dm
  complex(kind=dp)       :: exqq,argcd
...
function gfunc(nub,dm)
  implicit none
  complex(kind=dp)   ::  gfunc,nub,argcd
  real(kind=dp)              :: dm
  argcd = nub*dm
...
function h1func(nua,nub,dm)
  implicit none
  complex(kind=dp)       :: h1func,nua, nub
  real(kind=dp)                  :: dm
  complex(kind=dp)       :: argcd,exqq
...
function h2func(nua,nub,dm)
  implicit none
  complex(kind=dp)       :: h2func,nua,nub
  real(kind=dp)                  :: dm 
  complex(kind=dp)       :: argcd,exqq,exqp
...
subroutine getdcdh(om2,wvno,wvno2,fac)
  use RayleighWaveModel,only     :mmax,xmu,ur,uz,tr,tz,&
                                  iwat,zrho,xlam,dcdh
  implicit none
  real(kind=dp)                ::om2,wvno,wvno2,fac

  !internal variables
  real(kind=dp)                :: tur, tuz, ttz, ttr
  real(kind=dp)                :: dfac, gfac1, gfac2, gfac3, &
                                    gfac4, gfac5, gfac6
  real(kind=dp)                :: drho, dmu, dlm
  real(kind=dp)                :: duzdzp, dlur2, xl2mp, xl2mm
  real(kind=dp)                :: dl2mu, duzdzm, drur2
  real(kind=dp)                :: URB, DURDZM, DURDZP
  integer                :: m
...
subroutine getmat(m,wvno,om,a12, a14, a21, a23, &
  ...
  implicit none
  real(kind=dp)        :: wvno,om,a12, a14, a21, a23
  real(kind=dp)        :: ah,av,bh,bv,eta,rho
  real(kind=dp)        :: TA,TC,TF,TL,TN
  integer        :: m, iwat
...
subroutine sprayl(om,c,mmax,csph,usph,ugr)
  ...
  use RayleighWaveModel,only:vtp,dtp,dcda,dcdb,&
                              dcdh,dcdr,rtp,dcdgc,dcdgs
  implicit none
  real(kind=dp)           :: om,c,csph,usph,ugr
  integer           :: mmax
  ...
  do i=1,mmax
    ...
    dcdgc(i)=dcdgc(i)* vtp(i) / (tm**3)
    dcdgs(i)=dcdgs(i)* vtp(i) / (tm**3)
  enddo
...
subroutine sregn96(thk,vp,vs,rhom,nlayer,&
  ...
  implicit none
  integer        :: nlayer,iflsph
  real(kind=cr),INTENT(IN)    :: thk(nlayer),vp(nlayer),rhom(nlayer),&
                                  vs(nlayer)
  real(kind=dp),intent(inout):: dispu(nlayer),dispw(nlayer),&
                                stressu(nlayer),stressw(nlayer),&
                                  dc2da(nlayer),dc2db(nlayer),dc2dh(nlayer),&
                                  dc2dr(nlayer)
  real(kind=dp)              :: t,cp,cg

  !local
  integer              :: i
  integer              :: nsph
  ! real(kind=dp),PARAMETER    :: pi = 3.1415926535898
  real(kind=dp)              :: c, omega, wvno, gammar, csph, usph,twopi,sums
...
subroutine sregn96_hti(thk,vp,vs,rhom,nlayer,&
                  t,cp,cg,dispu,dispw,stressu,stressw,&
  ...
  real(kind=dp),intent(inout):: dispu(nlayer),dispw(nlayer),&
                                stressu(nlayer),stressw(nlayer),&
                                  dc2da(nlayer),dc2db(nlayer),dc2dh(nlayer),&
                                  dc2dr(nlayer),dc2dgc(nlayer),dc2dgs(nlayer)
  real(kind=dp)              :: t,cp,cg
...
subroutine sregnpu(thk,vp,vs,rhom,nlayer,&
  ...
  real(kind=dp),intent(inout):: dispu(nlayer),dispw(nlayer),&
                                  stressu(nlayer),stressw(nlayer)
  real(kind=dp),intent(inout):: dc2da(nlayer),dc2db(nlayer),dc2dh(nlayer),dc2dr(nlayer)
  real(kind=dp),intent(inout):: du2da(nlayer),du2db(nlayer),du2dh(nlayer),du2dr(nlayer)
  real(kind=dp)              :: t,cp,cg
  real(kind=dp),intent(inout):: t1,t2,cp1,cp2 ! period and phasev at slightly differentperiod

  !local
  integer              :: i
  integer              :: nsph
  ! real(kind=dp),PARAMETER    :: pi = atan(1.0) * 4.0
  real(kind=dp)              :: c, omega, wvno,twopi,sums
  real(kind=dp)              :: cg1,cg2,uc1
  real(kind=dp),ALLOCATABLE  :: dc2da1(:),dc2da2(:),dc2db1(:),dc2db2(:),&
                                dc2dh1(:),dc2dh2(:),dc2dr1(:),dc2dr2(:)
  real(kind=dp)              :: ar,tm,tm1
